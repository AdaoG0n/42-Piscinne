...
--- Explanation ---
    ...
    Function Declaration:
        void ft_putchar(char c): 
	This line declares a function named ft_putchar. The function takes one argument, c, which 
	is a character (char). The function does not return any value (void).

    Writing a Character:
        write(1, &c, 1): 
	This line uses the write() function to output the character passed to ft_putchar. 
	    The parameters are:
...
Português:
    ...
    Declaração da Função:
        void ft_putchar(char c): 
	Esta linha declara uma função chamada ft_putchar. A função aceita um argumento, c, 
	que é um caractere (char). A função não retorna nenhum valor (void).

    Escrevendo um Caractere:
        write(1, &c, 1): 
	Esta linha usa a função write() para enviar o caractere passado para ft_putchar. 
	    Os parâmetros são:
